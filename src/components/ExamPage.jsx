    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col transition-colors duration-200">
      <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 sticky top-0 z-10">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className={`flex items-center gap-2 px-4 py-1.5 rounded-lg font-medium ${
            timeLeft <= 300
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 flex gap-8">
        <div className="flex-1 flex flex-col min-h-[600px]">
          <div className="flex-1 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-8 relative flex flex-col">
            ...
            <div className="flex justify-between items-center mb-8">
              ...
              >
                <Flag size={16} className={flagged.has(currentIndex) ? 'fill-current' : ''} />
              ...
              {question.options.map((opt, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {isSelected && <div className="h-3 w-3 rounded-full bg-blue-600" />}
                    </div>
                    <span className={`text-[15px] ${isSelected ? 'text-blue-900 dark:text-blue-100 font-medium' : 'text-slate-700 dark:text-slate-300'}`}>
                      {opt}
                    </span>
                  </div>
                );
        ...
        <div className="w-[340px] flex flex-col gap-6">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 flex-1 flex flex-col">
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-4">Question Navigation</h3>
            ...
            <div className="grid grid-cols-5 gap-2.5 overflow-y-auto pr-2 pb-2 custom-scrollbar">
              {MOCK_QUESTIONS.map((_, idx) => {
                ...
                let btnClass = 'h-10 w-full rounded font-medium text-sm flex items-center justify-center transition-all relative border ';

                if (idx === currentIndex) {
                  btnClass += 'bg-blue-600 text-white border-blue-600 shadow-md transform scale-105';
                } else if (status === 'flagged') {
                  btnClass += 'bg-amber-50 text-amber-700 border-amber-300 dark:bg-amber-900/20 dark:text-amber-500 dark:border-amber-700';
                } else if (status === 'answered') {
                  btnClass += 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800';
                } else {
                  btnClass += 'bg-slate-50 text-slate-600 border-slate-200 hover:border-slate-300 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700 dark:hover:border-slate-600';
                }

                return (
                  <button key={idx} onClick={() => setCurrentIndex(idx)} className={btnClass}>
                    {status === 'flagged' && (
