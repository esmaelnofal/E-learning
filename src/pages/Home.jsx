...
export default function Home({ isDarkMode, toggleDarkMode }) {
  ...
  return (
    <div
      className={`app-main-wrapper min-h-screen ${isDarkMode ? "dark bg-slate-950 text-slate-100" : "bg-slate-100 text-slate-900"}`}
    >
          ...
          <div className="page-content px-8 py-8">
            <div className="page-header mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                >
                  Manage your enterprise learning environment.
                </p>
              </div>
              <button
                className={`rounded-md px-4 py-2 font-semibold transition ${isDarkMode ? "bg-slate-800 border border-slate-700 text-slate-100 hover:bg-slate-700" : "bg-white border border-slate-200 text-slate-900 hover:bg-slate-50"}`}
              >
                Export Report
              </button>
            </div>
